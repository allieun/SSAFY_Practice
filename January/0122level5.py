...
# (데이터 생략: user_data, blood_types, black_list는 동일)
...
def is_validation(user_data):
    ...
    # 4. 이름 길이 확인 (2~30자 사이가 아니면 오류)
    name_len = len(user_data['name'])
    if name_len < 2 or name_len > 30:
        wrong.append('name')
...
def create_user(user_data_list):
    ...
    for user in user_data_list:
        ...
        if result == 'blocked':
            # 'blocked'는 카운트만 하고 user_list에 넣지 않음
            wrong_count += 1
            continue 
            ...
            # False 반환 케이스 (데이터 오류)
            wrong_count += 1
            wrong_fields = result[1] # (False, ['field1', ...]) 에서 리스트 추출
    ...
    print(f'잘못된 데이터로 구성된 유저의 수는 {wrong_count} 입니다.')
    return user_list


# 함수를 호출하고 그 결과를 result 변수에 담습니다.
